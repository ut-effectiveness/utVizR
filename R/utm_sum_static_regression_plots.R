#' utm_sum_static_regression_plots () : Creates static Regression Plots for sum (fte)
#'
#' This function generates multiple static regression plots by transforming the data and applying
#' the `utm_sum_static_regression_plot()` function to each subset of data, identified by unique list names.
#'
...
utm_sum_static_regression_plots <- function(.data){
  ...
  output_graph <- purrr::map(df_list, ~utm_sum_static_regression_plot(.data = df, .))
