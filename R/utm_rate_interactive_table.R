#' utm_rate_interactive_table() : Modify data frame in order to make it compatible for 'utm_rate_shiny_table()
#' function.'
#'
#' This function achives the following:
#'  It renames the column "graph_name" to category,
#'  Filters data_frame so that it matches the provided "list_name" and then selects "title_name", "year", "outcome_rate", "category"
#'  It extracts the Title from the first row of "title_name" column, removes the the column and modifies the Title
#'
#'
#'
#'
#' @param .data a data frame made from df_rate_metric_values.rda
#' @param list_name a distinct var in df_rate_metric_values.rda
#' @export
#'
...
utm_rate_interactive_table <- function(.data, list_name) {

  data_frame <- .data %>%
    rename(category = graph_name)
  selected_data <- data_frame[data_frame$list_name == list_name, c("title_name", "year", "outcome_rate", "category")]
