#'  utm_sum_shiny_table() : Create sum table for "utMetrics Dashboard" shiny app.
#'
#'  This function effectively shows the sum table in the utMetrics dashboard. It takes filtered data frame
#'  as input, creates title and used the function utm_sum_interactive_table()' to produce the table.
#'
#' @param .data A data frame from df_count_metric_values.rda
#' @export
#'
#' @return A table for sum (fte). It consists of three columns: Year, sum and Category.
#'
...
utm_sum_shiny_table <- function(.data) {
  ...
  new_table <- utVizR::utm_sum_interactive_table(.data =step_3, list_name =step_3$list_name)
